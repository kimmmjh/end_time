...
class DataGenerator:
    ...
    def generate_batch(self, use_qmc: bool, device: torch.device) -> tuple[Tensor, Tensor, csr_matrix] | tuple[Tensor, Tensor]:
        ...
        if self.noise_model in ["circuit", "phenomenological"]:
            ...
            dets_z, obs_z = self.sampler_z.sample(shots=half, separate_observables=True)
            
            # X-basis samples (Observes Logical X, Errors Z)
            dets_x, obs_x = self.sampler_x.sample(shots=other, separate_observables=True)
            
            # Pad to match total logicals (usually 4)
            num_logicals = self.logicals.shape[0]
            
            if obs_z.shape[1] < num_logicals:
                obs_z = np.pad(obs_z, ((0,0), (0, num_logicals - obs_z.shape[1])), 'constant')
            
            if obs_x.shape[1] < num_logicals:
                obs_x = np.pad(obs_x, ((0,0), (0, num_logicals - obs_x.shape[1])), 'constant')
            
            # Combine
            detectors = np.concatenate([dets_z, dets_x], axis=0)
            observables = np.concatenate([obs_z, obs_x], axis=0)
