...
def generate_stim_circuit(
    code: StabilizerCode, rounds: int, p: float, q: float, basis: str = "Z"
) -> stim.Circuit:
    c = stim.Circuit()
...
def generate_phenomenological_circuit(
    code: StabilizerCode, rounds: int, p: float, q: float, basis: str = "Z"
) -> stim.Circuit:
    """
    ...
    for round_i in range(rounds):
        ...
        for s_idx, op in enumerate(ordered_stabs):
            ...
            for k, q_loc in enumerate(q_locs):
                ...
                q_idx = qubit_map[q_loc]
                if pauli == "X":
                    full_mpp_targets.append(stim.target_x(q_idx))
                elif pauli == "Z":
                    full_mpp_targets.append(stim.target_z(q_idx))
                elif pauli == "Y":
                    full_mpp_targets.append(stim.target_y(q_idx))
            ...
            for i in range(num_stab):
                ...
                if basis == "Z":
                    if types[i] == "vertex":
                        ...
                        c.append(
                            "DETECTOR", [current_rec, current_rec], [0.0] * 3
                        )  # Dummy detector
                else:  # basis == "X"
                    if types[i] == "face":
                        c.append("DETECTOR", [current_rec], [0.0] * 3)
